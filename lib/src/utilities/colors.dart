import 'package:flutter/material.dart';

class AppColors {
  static int colorTheme = 0xFF656CE4;
  // static int greyShade = 0xFF969696;
  static int lightGrey = 0xFFFAFAFA;
  static int background = 0xFFFCFAFA;
  static int greenShade = 0xFF07B642;
  static int greenShade1 = 0xFF0CBC8B;

  static const themeColor = Color(0xFF656CE4);
  static const grey1 = Color(0xFF969696);
  static const grey2 = Color(0xFF000FFF);
  static const greyLight = Color(0xFFFAFAFA);
  static const bg = Color(0xFFFCFAFA);
  static const shadeGreen = Color(0xFF07B642);
  static const shade1Green = Color(0xFF0CBC8B);
  static const greyMainText = Color(0XFFFFFFFF);
  static const greyTextColor = Color(0xFFA5A5A5);
  static const appGradient1 = Color(0XFF7292CF);
  static const appGradient2 = Color(0xFF2855ae);
  static const buttonCenterIconColor = Color(0xFF6789CA);

  static const blueBgResultCard = Color(0xFFE6EFFF);
  static const marksColor = Color(0xFF3A3A3A);

  static const purpleBlue = Color(0xFF6688CA);

  static const blueShade = Color(0XFF6184C7);
  static const greyShade = Color(0xFFCACACA);

  static const greyShade1 = Color(0xFFF5F6FC);
  static const yellowish = Color(0xFFFCF3E2);
  static const lightPurple1 = Color(0xFFFFD8FF);
  static const blueBorder = Color(0xFF5278C1);

  static const grey7777 = Color(0xFF777777);
  static const greyDivider = Color(0xFFE1E3E8);
}
